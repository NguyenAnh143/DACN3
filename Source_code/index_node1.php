...
if (isset($_GET['action']) && $_GET['action'] === 'fetchData') {
    ...
    $sql = "SELECT * FROM sensor_datanode1 WHERE timestamp >= NOW() - INTERVAL ? HOUR ORDER BY timestamp ASC";
    ...
    $sql_latest = "SELECT * FROM sensor_datanode1 ORDER BY timestamp DESC LIMIT 1";
...
<body>
    ...
    $sql_forecast = "SELECT * FROM forecast_results_node1 ORDER BY id DESC LIMIT 24";
    ...
    <script>
        ...
        function toggleDevice(type, button) {
            ...
            const nodeRedUrl = `http://${hostname}:1880/node1/control`;
        ...
        document.addEventListener('DOMContentLoaded', function() {
            const ws = new WebSocket('ws://' + window.location.hostname + ':1880/ws/node1/state');
